...
//密码需要6-20位数字或字母
            ...
            [manager GET:VerificationUrlString parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
                //请求成功，通过返回的信息判断当前手机号是否已经注册
                NSLog(@"responseObject：%@",responseObject);
                if ([operation.responseString isEqualToString:@"false"]) {
                    //用户已经注册
